...
//    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, KSys_NavAndStatus_Height, KSys_Screen_Width, KSys_Screen_Height - KSys_NavAndStatus_Height)];
    ...
    NSArray *titleArray = @[@"自定义我的页面-DemoOne",@"带有header和footer的table-DemoTwo",@"cell带有按钮、输入框-DemoThree",
                            @"服务端返回的数据页面-DemoFour",
                            @"cell带有动画的页面",
                            @"服务端数据带有type，根据type进行绑定不同cell",
                            @"带有tableHeaderView",@"长按移动单元格",@"删除单元格"];
//    NSArray *controllerArray = @[@""];
    ...
    for (int i = 0; i < titleArray.count; i ++) {
        ...
        {
            cellOneModel.pushControllerStr = @"ZKQuickTableDemoThreeVC";
        }else if (i == 3)
        {
            cellOneModel.pushControllerStr = @"ZKQuickTableDemoFourVC";
        }
